...
public class VRegistroGrupo extends JPanel implements ActionListener {
    ...
    private void generarDiasSemanas() {
        ...
        for (int i = 0; i < chDiasSemana.length; i++) {
            chDiasSemana[i].addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
        for (int i = 0; i < chDiasSemana.length; i++) {
                if (e.getSource() == chDiasSemana[i]) {
                    if (chDiasSemana[i].isSelected()) {
                        conDias++;
                    } else {
                        if (conDias == 4) {
                            for (JCheckBox cb : chDiasSemana) {
                                cb.setEnabled(true);
                            }
                        }
                        conDias--;
